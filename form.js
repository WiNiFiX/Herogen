...
function resistance_modifier(f, index)
{
	...
	if (ability_score[index] <= 4)
		...
		rm = 5
	if (f.i_12.selectedIndex > 0 && f.i_12.options[f.i_12.selectedIndex].text == ability[index])
		rm++
	if (index == strength)
	{
...
function update_skill_points()
{
	...
	d.open()
	d.writeln('<html>')
	d.writeln('<head><style>')
	d.writeln('*{margin:0;padding:0;box-sizing:border-box}')
	d.writeln('html,body{height:100%;overflow:hidden}')
	d.writeln('body{background:#1e293b;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;display:flex;align-items:center;justify-content:center;height:100%}')
	d.writeln('.sp{font-size:12px;font-weight:600;color:#94a3b8;text-align:center;line-height:1.4}')
	d.writeln('.sp .num{font-size:16px;font-weight:700;color:#3b82f6}')
	d.writeln('.sp .lbl{font-size:10px;color:#475569;letter-spacing:0.05em;text-transform:uppercase;display:block;margin-top:1px}')
	d.writeln('</style></head>')
	d.writeln('<body>')
	d.write  ('<div class="sp">')
	d.write  ('<span class="num" title="Skill points spent">' + skill_points_spent + '</span>')
	d.write  (' <span style="color:#475569">of</span> ')
	d.write  ('<span class="num" title="Total skill points available">' + skill_points_available + '</span>')
	d.write  ('<span class="lbl">Skill Points</span>')
	d.writeln('</div>')
	d.writeln('</body>')
	d.writeln('</html>')
	d.close()
